...
$('.show-sidebar-left').click(function(){
    if($('.slide-showing').length > 0){
        $('.main').removeClass('slide-showing');
    }else{
        $('.main').addClass('slide-showing');
    }
})
...
let array = [
    ...
    },
    {
        ...
        'cs': 'vHot',
        'class': 'type home status_show hot'
    },
    {
        "category": "站长推荐",
        "count": 0,
   ...
   let data_list = array.concat(data.data);
    data_list.forEach((ty, index) => {
        ...
        if(ty.id === 81 || ty.id === 82 || ty.id === 83){
            $('#navUl').append(`
                <li>
                <a id="${ty.cs}" class="" href="${ty.url}" rel="Home" title="Home">
                    <span>${ty.category}</span>
                </a>
            </li>
            `)
        } else {
            $('#navUl').append(`
            <li>
                <a id="navLi-a${ty.id}" class="" href="../videoType.html?parameter=$${parameter}" rel="Home" title="Home">
                    <span>${ty.category}</span>
                </a>
            </li>
            `)
...
$.get(apiSuffix.vLinks, {
    ...
    if (data['code'] == 0) {
            data.data.forEach(ad => {
                vLink.append(`
                    <a style="display: inline-block;margin: 0 10px;" href="http://${ad.url}" target="_blank"><i class="glyphicon glyphicon-ok"></i>${ad.name}</a>
                `);
            });
    }
});
