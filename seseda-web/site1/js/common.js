   ...
   let data_list = array.concat(data.data);
    data_list.forEach((ty, index) => {
        ...
        if(ty.id === 81 || ty.id === 82 || ty.id === 83){
            $('#navUl').append(`
            <li class="drop" id=${ty.cs}>
                <a href="${ty.url}">${ty.category}</a>
            </li>`)
            $('#m_nav').append(`
            <li><a href="${ty.url}">${ty.category}</a></li>`)
        } else{
            $('#navUl').append(`
            <li class="drop" id="nav${ty.id}">
                <a href="videoType.html?parameter=$${parameter}">${ty.category}</a>
            </li>`)
            $('#m_nav').append(`
            <li data_id=${ty.id}><a href="videoType.html?parameter=$${parameter}">${ty.category}</a></li>`)
        }
    });
});
...
$.get(apiSuffix.vTags, {
    ...
    data.data.forEach((ty, index) => {
        ...
        //左侧菜单栏
        $('#vTags_m').append(`
            <a class="btn" title="${ty.name}" href="videoType.html?parameter=$${parameter}">${ty.name}</a>
        `)
    });
});
