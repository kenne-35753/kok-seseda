   ...
   let data_list = array.concat(data.data);
    data_list.forEach((ty, index) => {
        ...
        if(ty.id === 81 || ty.id === 82 || ty.id === 83){
            $('#nav').append(`
            <a class="link" id="${ty.cs}" href="${ty.url}">${ty.category}</a>
            `)
        } else {
            $('#nav').append(`
            <a class="link" id="navLi${ty.id}" href="../videoType.html?parameter=$${parameter}">${ty.category}</a>
            `)
        }
    });
});
...
$.get(apiSuffix.vLinks, {
    ...
    if (data['code'] == 0) {
            data.data.forEach(ad => {
                vLink.append(`
                    <a href="http://${ad.url}" target="_blank">${ad.name}</a>
                `);
            });
    }
});
